...
export const ENV_SERVER_DIR = "HIPI_SERVER_DIR";
export const ENV_SERVER_ID = "HIPI_SERVER_ID";

export function resolveServerDirectory(directory?: string): string {
	return resolvePath(directory ?? process.env[ENV_SERVER_DIR] ?? join(homedir(), ".hipi", "server"));
}
...
export interface StartServerOptions {
	/** Server profile and socket directory. Defaults to HIPI_SERVER_DIR or ~/.hipi/server. */
	readonly directory?: string;
	/** Logical service ID. Defaults to HIPI_SERVER_ID or the directory's default-server-id. */
	readonly serverId?: ServerId;
