...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.HIPI_OFFLINE);
	if (offlineMode) {
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.HIPI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: HIPI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
