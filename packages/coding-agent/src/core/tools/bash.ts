...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect HIPI_* environment variables for current model and session details."],
} as const;
...
function resolveSpawnContext(
	...
	delete env.HIPI_SESSION_ID;
	delete env.HIPI_SESSION_FILE;
	delete env.HIPI_PROVIDER;
	delete env.HIPI_MODEL;
	delete env.HIPI_REASONING_LEVEL;
	if (exposeSessionEnvironment && ctx) {
		const model = ctx.model;
		env.HIPI_SESSION_ID = ctx.sessionManager.getSessionId();
		const sessionFile = ctx.sessionManager.getSessionFile();
		if (sessionFile) env.HIPI_SESSION_FILE = sessionFile;
		if (model) {
			env.HIPI_PROVIDER = model.provider;
			env.HIPI_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.HIPI_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current HipiWork session metadata as HIPI_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;
