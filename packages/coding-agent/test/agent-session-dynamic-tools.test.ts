...
describe("AgentSession dynamic tool registration", () => {
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(session.systemPrompt).toContain(
			"You can inspect HIPI_* environment variables for current model and session details.",
		);
		await bashTool.execute("bash-env", { command: "printf ok" });
		expect(sessionEnv).toMatchObject({
			HIPI_SESSION_ID: session.sessionId,
			HIPI_SESSION_FILE: session.sessionFile,
			HIPI_PROVIDER: model.provider,
			HIPI_MODEL: model.id,
			HIPI_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("HIPI_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("HIPI_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("HIPI_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("HIPI_MODEL");
		expect(optedOutEnv).not.toHaveProperty("HIPI_REASONING_LEVEL");
