...
const originalOffline = process.env.HIPI_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.HIPI_OFFLINE;
	else process.env.HIPI_OFFLINE = originalOffline;
	vi.unstubAllGlobals();
});
