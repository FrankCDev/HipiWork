...
// Self-update has no built-in release feed, so tests inject one and mock it.
const VERSION_CHECK_URL = "https://releases.example.test/api/latest-version";
...
`,
		...
		vi.stubEnv("HIPI_INSTALLER_API_BASE", "https://example.test/api/installer/releases");
		vi.stubEnv("HIPI_MANAGED_INSTALL_ROOT", managedRoot);
	...
	beforeEach(() => {
		allowNetwork();
		vi.stubEnv("HIPI_LATEST_VERSION_URL", VERSION_CHECK_URL);
		...
		originalPiPackageDir = process.env.HIPI_PACKAGE_DIR;
	...
	afterEach(() => {
		...
		if (originalPiPackageDir === undefined) {
			delete process.env.HIPI_PACKAGE_DIR;
		} else {
			process.env.HIPI_PACKAGE_DIR = originalPiPackageDir;
		}
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		const previousSkipVersionCheck = process.env.HIPI_SKIP_VERSION_CHECK;
		process.env.HIPI_SKIP_VERSION_CHECK = "1";
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) {
				delete process.env.HIPI_SKIP_VERSION_CHECK;
			} else {
				process.env.HIPI_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			}
		}
	});
	...
	it("retries a transient self-update version check", async () => {
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) delete process.env.HIPI_SKIP_VERSION_CHECK;
			else process.env.HIPI_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	});
	...
	it("keeps npm self-updates non-managed when the managed environment is inherited", async () => {
		...
		vi.stubEnv("HIPI_MANAGED_INSTALL_ROOT", inheritedManagedRoot);
