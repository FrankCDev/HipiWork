...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.HIPI_TELEMETRY;
		delete process.env.HIPI_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.HIPI_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/FrankCDev/HipiWork");
		expect(headers?.["X-OpenRouter-Title"]).toBe("hipi");
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe("hipi");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("hipi");
	});
