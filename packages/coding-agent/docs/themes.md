> hipi can create themes. Ask it to build one for your setup.
...
hipi loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.hipi/agent/themes/*.json`
- Project: `.hipi/themes/*.json` (only after the project is trusted)
...
On first run, hipi detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.hipi/agent/themes
vim ~/.hipi/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, hipi reloads it automatically for immediate visual feedback.
...
hipi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, hipi falls back to the nearest approximation.
