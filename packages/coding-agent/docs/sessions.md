...
hipi saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.hipi/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.
...
hipi --session <path|id> # Use a specific session file or partial session ID
hipi --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `hipi -r` opens the same picker at startup.
...
When available, hipi uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
hipi --name "Refactor auth module"
hipi --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `hipi -r`.
...
When `/tree` switches away from one branch to another, hipi can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
