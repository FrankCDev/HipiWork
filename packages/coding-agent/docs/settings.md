...
hipi uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.hipi/agent/settings.json` | Global (all projects) |
| `.hipi/settings.json` | Project (current directory) |
...
| `enableInstallTelemetry` | boolean | `true` | Send selected provider attribution headers. This does not control version checks |
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`HIPI_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so hipi resumes after the editor exits:
...
`enableInstallTelemetry` controls the attribution headers sent with OpenRouter, NVIDIA NIM, and Cloudflare provider requests. Opting out drops those headers.

Version checks are off unless `HIPI_LATEST_VERSION_URL` points at a release feed, so HipiWork makes no startup request to any external service by default.

Set `HIPI_SKIP_VERSION_CHECK=1` to disable the version check even when a release feed is configured. Use `--offline` or `HIPI_OFFLINE=1` to disable all startup network operations described here, including version checks and package update checks.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before hipi sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.hipi/agent/npm/`; project-scoped npm packages install under `.hipi/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, hipi uses its standard defaults |
...
```json
{ "sessionDir": ".hipi/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `HIPI_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.hipi/agent/settings.json` resolve relative to `~/.hipi/agent`. Paths in `.hipi/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
...
Project settings (`.hipi/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.hipi/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .hipi/settings.json (project)
{
