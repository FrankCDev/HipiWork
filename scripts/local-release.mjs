...
// The coding-agent package publishes its CLI under this name (see its package.json "bin").
// The isolated installs below only expose node_modules/.bin, so the outer shim mirrors it.
const CLI_BIN = "hipi";
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `hipi-${platform}.zip` : `hipi-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, `${CLI_BIN}.cmd`))) {
			writeFileSync(
				join(installDirectory, `${CLI_BIN}.cmd`),
				`@ECHO off\r\n"%~dp0node_modules\\.bin\\${CLI_BIN}.cmd" %*\r\n`,
			);
			writeFileSync(join(installDirectory, `${CLI_BIN}.ps1`), `& "$PSScriptRoot/node_modules/.bin/${CLI_BIN}.ps1" @args\n`);
			return;
		}
		writeFileSync(join(installDirectory, `${CLI_BIN}.cmd`), `@ECHO off\r\n"%~dp0node_modules\\.bin\\${CLI_BIN}.exe" %*\r\n`);
		writeFileSync(join(installDirectory, `${CLI_BIN}.ps1`), `& "$PSScriptRoot/node_modules/.bin/${CLI_BIN}.exe" @args\n`);
		return;
	}
	symlinkSync(join("node_modules", ".bin", CLI_BIN), join(installDirectory, CLI_BIN));
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `hipi-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "hipi.exe" : "hipi")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? `${CLI_BIN}.cmd` : CLI_BIN)} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? `${CLI_BIN}.cmd` : CLI_BIN)} --help`);
	}
}
