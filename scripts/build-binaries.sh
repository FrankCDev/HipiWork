#!/usr/bin/env bash
#
# Build hipi binaries for all platforms locally.
...
#     hipi-darwin-arm64.tar.gz
#     hipi-darwin-x64.tar.gz
#     hipi-linux-x64.tar.gz
#     hipi-linux-arm64.tar.gz
#     hipi-windows-x64.zip
#     hipi-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before hipi starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/hipi.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/hipi"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating hipi-$platform.zip..."
        (cd "$platform" && zip -r ../hipi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating hipi-$platform.tar.gz..."
        mv "$platform" hipi && tar -czf hipi-$platform.tar.gz hipi && mv hipi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../hipi-$platform.zip)
    else
        tar -xzf hipi-$platform.tar.gz && mv hipi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/hipi.exe"
    else
        echo "  $OUTPUT_DIR/$platform/hipi"
    fi
done
