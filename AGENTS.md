...
- Pre-commit blocks lockfile commits unless `HIPI_ALLOW_LOCKFILE_CHANGE=1`. Don't bypass unless the user wants the lockfile change committed.

## Git

Multiple hipi sessions may be running in this cwd at the same time, each modifying different files. Git operations that touch unstaged, staged, or untracked files outside your own changes will stomp on other sessions' work. Follow these rules:
...
## Testing hipi Interactive Mode with tmux

For testing hipi's interactive mode, load and follow [.hipi/skills/interactive-testing.md](.hipi/skills/interactive-testing.md).
...
For release preparation, publishing, verification, or recovery, load and follow [.hipi/skills/release.md](.hipi/skills/release.md).
